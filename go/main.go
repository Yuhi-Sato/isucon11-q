...
func postInitialize(c echo.Context) error {
	...
	var isus []Isu
	err = db.Select(&isus, "SELECT `jia_isu_uuid`, `image` FROM `isu`")
	if err != nil {
		c.Logger().Errorf("db error : %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
	...
	for _, isu := range isus {
		filepath := fmt.Sprintf("../public/%s.jpg", isu.JIAIsuUUID)
...
func postIsu(c echo.Context) error {
	...
	filepath := fmt.Sprintf("../public/%s.jpg", jiaIsuUUID)
...
func getIsuIcon(c echo.Context) error {
	...
	// var image []byte
	// image, err = os.ReadFile(fmt.Sprintf("../public/%s.jpg", jiaIsuUUID))
	// if err != nil {
	...
	c.Response().Header().Set("X-Accel-Redirect", fmt.Sprintf("/%s.jpg", jiaIsuUUID))
...
func generateIsuGraphResponse(db *sqlx.DB, jiaIsuUUID string, graphDate time.Time) ([]GraphResponse, error) {
	...
	rows, err := db.Queryx("SELECT id, jia_isu_uuid, timestamp, is_sitting, `condition`, message, created_at FROM `isu_condition` WHERE `jia_isu_uuid` = ? ORDER BY `timestamp` ASC", jiaIsuUUID)
	if err != nil {
...
func getIsuConditionsFromDB(db *sqlx.DB, jiaIsuUUID string, endTime time.Time, conditionLevel map[string]interface{}, startTime time.Time,
	...
	var err error
	conditionLevels := make([]string, 0, len(conditionLevel))
	for k := range conditionLevel {
		conditionLevels = append(conditionLevels, k)
	}

	if startTime.IsZero() {
		query := "SELECT id, jia_isu_uuid, timestamp, is_sitting, `condition`, message, created_at, condition_level FROM `isu_condition` WHERE `jia_isu_uuid` = ?" +
			"	AND `timestamp` < ? AND condition_level IN (?)" +
			"	ORDER BY `timestamp` DESC LIMIT ?"

		query, args, err := sqlx.In(query, jiaIsuUUID, endTime, conditionLevels, limit)
		...
		query := "SELECT id, jia_isu_uuid, timestamp, is_sitting, `condition`, message, created_at, condition_level FROM `isu_condition` WHERE `jia_isu_uuid` = ?" +
			"	AND `timestamp` < ?" +
			"	AND ? <= `timestamp` condition_level IN (?)" +
			"	ORDER BY `timestamp` DESC LIMIT ?"

		query, args, err := sqlx.In(query, jiaIsuUUID, endTime, startTime, conditionLevels, limit)
		query = db.Rebind(query)
		if err != nil {
			return nil, fmt.Errorf("db error: %v", err)
		}
...
func getTrend(c echo.Context) error {
	res, ok := trendCache.Get("")
	if !ok {
		res, err := sf.Do("",
			func() ([]TrendResponse, error) {
				characterList := []Isu{}
		...
		trendCache.Set("", res, 800*time.Millisecond)
	}
